<?php

namespace IbnuSahid\StorageBackup\StorageAdapters;

use Illuminate\Support\Facades\Storage;

class AzureBlobStorageAdapter implements StorageAdapter
{
    public function store(string $sourcePath, string $destinationPath): bool
    {
        return Storage::disk('azure')->put($destinationPath, file_get_contents($sourcePath));
    }

    public function retrieve(string $path): string
    {
        return Storage::disk('azure')->get($path);
    }

    public function delete(string $path): bool
    {
        return Storage::disk('azure')->delete($path);
    }

    public function list(string $path): array
    {
        return Storage::disk('azure')->files($path);
    }

    public function exists(string $path): bool
    {
        return Storage::disk('azure')->exists($path);
    }
} 